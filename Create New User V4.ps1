...
$UserPath= "OU=New Users,OU=People,DC=corp,DC=edhc,DC=com"
...
#Function to add users to Base Groups
...
#Function to add users to Care Advocate Groups

    Function add-UserToCareAdvocateGroups{
        param(
        [String]$SamAccountName,
        [String]$LogFile
        )
    ...
    try
        ...
        if($UserAccountInfo)
            ...
            if($UserAccountInfo.title -eq "Care Advocate")
                        {
                        add-UserToCareAdvocateGroups -SamAccountName $UserAccountInfo.SamAccountName -LogFile $LogFile
                        }
                    else
                        {
  ...
  Start-Sleep -Seconds 5          
                ...
                if($UserAccountInfoPostCreation)
                    ...
                    if($UserTitlePostAccountCreation -eq "Care Advocate")
                        {
                        add-UserToCareAdvocateGroups -SamAccountName $UserAccountInfoPostCreation.SamAccountName -LogFile $LogFile
                        }
                    else
                        {
                        $BaseGroups = @("SG_EmployeeVPNAccess", "SG_KnowBe4", "SG_File_BaseAccess", "SG_DocuSign_EDHC_CA")
