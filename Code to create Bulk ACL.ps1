...
$CSVFilePath = "C:\Users\ext.goutham.gummadi\OneDrive - EDHC\Desktop\Birthrights\ACL Bulk upload.csv"
$GroupsData = Import-Csv $CSVFilePath

# Log file path
$LogFilePath = "C:\Users\ext.goutham.gummadi\OneDrive - EDHC\Desktop\Birthrights\ACL_Bulk_Upload_Log.txt"
...
foreach ($group in $GroupsData) {
    try {
        ...
        # Create the AD group
        New-ADGroup -Name $RBACName -Description $RBACDescription -GroupCategory Security -ManagedBy $Owner.SamAccountName -GroupScope Global -Path "OU=$RBACDepartment,OU=ACL,OU=Access Control,DC=corp,DC=edhc,DC=com"
